...
public class TcpNotificationClient
{
    ...
    private async Task ListenForNotificationsAsync()
    {
        ...
        while (_client.Connected)
        {
            try
            {
                ...
                ShowNotification("Обнаружена продувка с алкоголем.", message);
            }
            catch (Exception ex)
            {
    ...
    private void ShowNotification(string title, string message)
    {
        ...
        {
            var notificationWindow = new NotificationWindow(title, message, this);
            notificationWindow.Show();
        });
    }
    
    public void SendReadConfirmation(string message)
    {
